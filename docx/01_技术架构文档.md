...
以下与当前仓库实现一致，体现「从能跑到可演示、可观测、可收尾」：

| 主题 | 说明 |
|------|------|
| **对话 SSE** | `POST /chat` 流式返回规划阶段事件，前端展示实时进度；队列 **短超时 + 总时限**，避免长时间占用连接导致进程难以退出。 |
| **LLM 健壮性** | 完成接口 **超时重试**、`max_tokens` 控制；降低「卡在调用模型」的概率。 |
| **中转性能** | `train_service` 侧 **hub 并行查询**、控制 hub 数量与线程池，缩短中转方案等待。 |
| **快捷回复协议** | 正文中的 `::actions::["按钮1","按钮2"]` 支持**文中任意位置**解析（不要求必须在文末），前后文拼接为展示用正文；前后端解析逻辑对齐（`app/utils/quick_replies.py` + 前端 `parseQuickReplies.ts`）。 |
| **演示检票 URL** | `GET /api/v1/tickets/check-in/public/{order_no}`：访问即执行检票、返回 **品牌化 HTML 结果页**（含乘车人、订单号等）；与前端二维码编码同源，便于 Demo「像真实闸机扫 URL」。 |
| **Agent 话术约束** | 系统提示约束：有卡片时勿冗长复述车次表；勿说「详见**上方**卡片」（卡片在消息**下方**）；控制多日查票纪律等。 |
| **容器化（可选）** | 仓库提供 `Dockerfile` / `docker-compose.yml`，便于评委一键拉起环境（以实际文件为准）。 |

### 4.4 前端架构要点（Next.js）

| 主题 | 说明 |
|------|------|
| **Hydration** | `ClientShell` **首帧挂载门控**后再渲染业务树，避免 Zustand persist 与 SSR 首帧不一致；`ThemeProvider` 内 **延后挂载 `CssBaseline`**，避免 Emotion 全局 `<style>` 与骨架屏子树顺序触发 mismatch。 |
| **查票日期** | `SearchForm` 使用 `useSyncExternalStore` 模式的 **`useIsClient`**，首帧不调用 `getToday()` 参与 DOM，客户端再写入日期，消除时区/持久化差异导致的告警。 |
| **车次类型筛选** | 查询 **API 始终拉全量车次**；`G/D/...` 类型仅在 **前端 `searchStore` + 结果列表** 过滤，减少重复请求、职责清晰。 |
| **行程同步** | 行程页 **短间隔 refetch**；检票弹窗打开时 **invalidate 列表**，他人扫码或本机「立即检票」后尽快反映已检状态。 |
...
- Agent 不是噱头，而是核心决策引擎；
- 后端、前端、多端方案齐备；
- **SSE 可观测、快捷回复与卡片同源、演示检票 URL、前端工程化治理** 等细节可证明「能落地」而不止于概念；
- 合规意识明确，交易边界清晰；
- 具备从 Demo 走向产品化的演进路径。

**答辩叙事与演示顺序**建议直接配合 `docx/00_答辩与演示导读.md` 使用。
