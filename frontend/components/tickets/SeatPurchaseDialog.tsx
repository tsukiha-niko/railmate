...
export function SeatPurchaseDialog({
  ...
  return (
    <Dialog open={open} onClose={purchasing ? undefined : onClose} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 6 } }}>
      <DialogTitle sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
        {t("booking.dialog.title")}
        {demoMode && <Chip label={t("booking.demo.badge")} size="small" sx={{ borderRadius: 2.5 }} />}
      </DialogTitle>
      ...
      <DialogContent dividers>
        {purchasedOrder ? (
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            <Alert severity="success" icon={<Ticket size={20} />}>
              ...
              <Box sx={{ display: "flex", gap: 0.75, mt: 1, flexWrap: "wrap" }}>
                <Chip label={purchasedOrder.booking_reference} size="small" color="success" sx={{ borderRadius: 2.5 }} />
                <Chip label={`${purchasedOrder.coach_no ?? "--"} ${t("booking.success.coach")}`} size="small" variant="outlined" sx={{ borderRadius: 2.5 }} />
                <Chip label={purchasedOrder.seat_no ?? "--"} size="small" variant="outlined" sx={{ borderRadius: 2.5 }} />
              </Box>
            </Alert>
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            <Box sx={{ borderRadius: 4, border: 1, borderColor: (th: any) => `${th.palette.divider}60`, p: 2.5 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
                <Chip label={trainTypeLabel} size="small" variant="outlined" sx={{ borderRadius: 2.5 }} />
                <Chip label={trainNo} color="primary" size="small" sx={{ fontWeight: 700, borderRadius: 2.5 }} />
...
function InfoBlock({ label, value }: { label: string; value: string }) {
  return (
    <Box sx={{ borderRadius: 3, border: 1, borderColor: (th: any) => `${th.palette.divider}60`, bgcolor: "action.hover", p: 1.5 }}>
