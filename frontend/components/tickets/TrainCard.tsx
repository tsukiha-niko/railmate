...
export function TrainCard({ train, date, index = 0, tags = [] }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: Math.min(index * 0.04, 0.4) }}>
      <Card variant="outlined" sx={{ "&:hover": { borderColor: "primary.main", boxShadow: "0 14px 28px -22px rgba(15,23,42,0.5)" } }}>
        <CardActionArea
          component={Link}
        ...
        >
          <Box sx={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", gap: 1, mb: 1 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, flexWrap: "wrap" }}>
              ...
              {tags.map((tag) => <Chip key={tag} label={tag} size="small" color="warning" sx={{ fontSize: "0.625rem", height: 20 }} />)}
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
              {lowestFare && (
                <Chip label={`${t("tickets.fare.from")} ${formatPrice(lowestFare.price)}`} size="small" color="primary" variant="outlined" sx={{ fontWeight: 700, fontSize: "0.75rem" }} />
              )}
              <Chip
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: { xs: 1.5, sm: 2 } }}>
            <Box sx={{ flex: 1 }}>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mt: 0.25 }}>{train.from_station}</Typography>
            </Box>
            <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 0.25, px: 1 }}>
              ...
              <Box sx={{ display: "flex", alignItems: "center" }}>
                <Box sx={{ width: { xs: 32, sm: 56 }, height: "1px", background: "linear-gradient(to right, var(--border), var(--primary))" }} />
                <ArrowRight size={14} style={{ opacity: 0.5 }} />
              </Box>
            </Box>
            <Box sx={{ flex: 1, textAlign: "right" }}>
              <Typography variant="h5" fontWeight={800} sx={{ fontVariantNumeric: "tabular-nums" }}>{train.arrival_time}</Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mt: 0.25 }}>{train.to_station}</Typography>
            </Box>
          </Box>

          {additionalFares.length > 0 && (
            <Box sx={{ display: { xs: "none", sm: "flex" }, flexWrap: "wrap", gap: 0.75, mt: 1 }}>
              {additionalFares.map((fare) => (
                <Chip key={fare.key} label={`${getFareLabel(fare.key, fmtLocale)} ${formatPrice(fare.price)}`} size="small" variant="outlined" sx={{ fontSize: "0.75rem" }} />
              ))}
            </Box>
          )}
        </CardActionArea>
